...
| [`calendardeck`](calendardeck/) | Google Calendar events as flowdeck cards — one-way sync (GCal → local) with per-day, week, month, and year cards; `send-to-gcal` action for pushing tasks back | `calendardeck-init`, `calendardeck-sync` |
